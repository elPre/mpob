 ...
 */
public final class HlsMediaPeriodMute implements MediaPeriod, HlsSampleStreamWrapperMute.Callback,
    ...
    private HlsSampleStreamWrapperMute[] sampleStreamWrappers;
    private HlsSampleStreamWrapperMute[] enabledSampleStreamWrappers;
    private SequenceableLoader compositeSequenceableLoader;

    public HlsMediaPeriodMute(
        ...
        sampleStreamWrappers = new HlsSampleStreamWrapperMute[0];
        enabledSampleStreamWrappers = new HlsSampleStreamWrapperMute[0];
    }

    public void release() {
        playlistTracker.removeListener(this);
        continueLoadingHandler.removeCallbacksAndMessages(null);
        for (HlsSampleStreamWrapperMute sampleStreamWrapper : sampleStreamWrappers) {
    ...
    public long selectTracks(TrackSelection[] selections, boolean[] mayRetainStreamFlags,
        ...
        HlsSampleStreamWrapperMute[] newEnabledSampleStreamWrappers =
                new HlsSampleStreamWrapperMute[sampleStreamWrappers.length];
        for (int i = 0; i < sampleStreamWrappers.length; i++) {
            for (int j = 0; j < selections.length; j++) {
                childStreams[j] = streamChildIndices[j] == i ? streams[j] : null;
                childSelections[j] = selectionChildIndices[j] == i ? selections[j] : null;
            }
            HlsSampleStreamWrapperMute sampleStreamWrapper = sampleStreamWrappers[i];
    ...
    public void discardBuffer(long positionUs, boolean toKeyframe) {
        for (HlsSampleStreamWrapperMute sampleStreamWrapper : enabledSampleStreamWrappers) {
    ...
    @Override
    public void onContinueLoadingRequested(HlsSampleStreamWrapperMute sampleStreamWrapper) {
        if (trackGroups == null) {
            // Still preparing.
            return;
        }
    ...
    public void onPlaylistBlacklisted(HlsUrl url, long blacklistMs) {
        for (HlsSampleStreamWrapperMute streamWrapper : sampleStreamWrappers) {
    ...
    private void buildAndPrepareSampleStreamWrappers(long positionUs) {
        ...
        sampleStreamWrappers = new HlsSampleStreamWrapperMute[wrapperCount];
        ...
        for (int i = 0; i < audioRenditions.size(); i++) {
            ...
            HlsSampleStreamWrapperMute sampleStreamWrapper =
    ...
    private void buildAndPrepareMainSampleStreamWrapper(
        ...
        HlsSampleStreamWrapperMute sampleStreamWrapper = buildSampleStreamWrapper(C.TRACK_TYPE_DEFAULT,
    ...
    private HlsSampleStreamWrapperMute buildSampleStreamWrapper(int trackType, HlsUrl[] variants,
                                                            Format muxedAudioFormat, List<Format> muxedCaptionFormats, long positionUs) {
        HlsChunkSourceMute defaultChunkSource = new HlsChunkSourceMute(extractorFactory, playlistTracker,
                variants, dataSourceFactory, timestampAdjusterProvider, muxedCaptionFormats);
        return new HlsSampleStreamWrapperMute(trackType, this, defaultChunkSource, allocator, positionUs,
    ...
    private void continuePreparingOrLoading() {
        if (trackGroups != null) {
            ...
            for (HlsSampleStreamWrapperMute wrapper : sampleStreamWrappers) {
