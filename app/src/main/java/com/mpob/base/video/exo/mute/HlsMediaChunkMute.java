 ...
 */
 class HlsMediaChunkMute extends MediaChunk {
  ...
  private HlsSampleStreamWrapperMute output;
  ...
  /**
   * @param extractorFactory A {@link com.google.android.exoplayer2.source.hls.HlsExtractorFactory} from which the HLS media chunk extractor
   *     is obtained.
   ...
   * @param previousChunk The {@link HlsMediaChunkMute} that preceded this one. May be null.
  ...
  public HlsMediaChunkMute(
    ...
    this.isEncrypted = this.dataSource instanceof Aes128DataSourceMute;
  ...
  /**
   * Initializes the chunk for loading, setting the {@link HlsSampleStreamWrapperMute} that will receive
   * samples as they are loaded.
   *
   * @param output The output that will receive the loaded samples.
   */
  public void init(HlsSampleStreamWrapperMute output) {
    this.output = output;
  ...
  /**
   * If the segment is fully encrypted, returns an {@link Aes128DataSourceMute} that wraps the original
  ...
  private static DataSource buildDataSource(DataSource dataSource, byte[] fullSegmentEncryptionKey,
    ...
    if (fullSegmentEncryptionKey != null) {
      return new Aes128DataSourceMute(dataSource, fullSegmentEncryptionKey, encryptionIv);
    }
    return dataSource;
  }
