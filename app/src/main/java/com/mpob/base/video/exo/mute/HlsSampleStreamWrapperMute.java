...
/**
 * Created by hectorleyvavillanueva on 4/26/18.
 */

public class HlsSampleStreamWrapperMute  implements Loader.Callback<Chunk>,
     ...
     */
    public interface Callback extends SequenceableLoader.Callback<HlsSampleStreamWrapperMute> {
    ...
    private final HlsSampleStreamWrapperMute.Callback callback;
    private final HlsChunkSourceMute chunkSource;
    private final Allocator allocator;
    private final Format muxedAudioFormat;
    private final int minLoadableRetryCount;
    private final Loader loader;
    private final MediaSourceEventListener.EventDispatcher eventDispatcher;
    private final HlsChunkSourceMute.HlsChunkHolder nextChunkHolder;
    private final ArrayList<HlsMediaChunkMute> mediaChunks;
     ...
     * @param chunkSource A {@link HlsChunkSourceMute} from which chunks to load are obtained.
     ...
     */
    public HlsSampleStreamWrapperMute(int trackType, HlsSampleStreamWrapperMute.Callback callback, HlsChunkSourceMute chunkSource,
        ...
        nextChunkHolder = new HlsChunkSourceMute.HlsChunkHolder();
    ...
    /**
     * Called by the parent {@link HlsMediaPeriodMute} when a track selection occurs.
     *
     ...
     *     seeking disabled).
     * @return Whether this wrapper requires the parent {@link HlsMediaPeriodMute} to perform a seek as
    ...
    public boolean selectTracks(TrackSelection[] selections, boolean[] mayRetainStreamFlags,
        ...
        for (int i = 0; i < selections.length; i++) {
            if (streams[i] != null && (selections[i] == null || !mayRetainStreamFlags[i])) {
                ...
                ((HlsSampleStreamMute) streams[i]).unbindSampleQueue();
                streams[i] = null;
            }
        }
        ...
        for (int i = 0; i < selections.length; i++) {
            if (streams[i] == null && selections[i] != null) {
                ...
                streams[i] = new HlsSampleStreamMute(this, trackGroupIndex);
    ...
    public int readData(int sampleQueueIndex, FormatHolder formatHolder, DecoderInputBuffer buffer,
        ...
        if (!mediaChunks.isEmpty()) {
            ...
            HlsMediaChunkMute currentChunk = mediaChunks.get(0);
    ...
    public long getBufferedPositionUs() {
        if (loadingFinished) {
            ...
            HlsMediaChunkMute lastMediaChunk = getLastMediaChunk();
            HlsMediaChunkMute lastCompletedMediaChunk = lastMediaChunk.isLoadCompleted() ? lastMediaChunk
    ...
    public boolean continueLoading(long positionUs) {
        ...
        HlsMediaChunkMute previousChunk;
        long loadPositionUs;
        if (isPendingReset()) {
            previousChunk = null;
        ...
        if (isMediaChunk(loadable)) {
            pendingResetPositionUs = C.TIME_UNSET;
            HlsMediaChunkMute mediaChunk = (HlsMediaChunkMute) loadable;
    ...
    public int onLoadError(Chunk loadable, long elapsedRealtimeMs, long loadDurationMs,
        ...
        if (chunkSource.onChunkLoadError(loadable, cancelable, error)) {
            if (isMediaChunk) {
                HlsMediaChunkMute removed = mediaChunks.remove(mediaChunks.size() - 1);
    ...
    private boolean finishedReadingChunk(HlsMediaChunkMute chunk) {
    ...
    /**
     * Builds tracks that are exposed by this {@link HlsSampleStreamWrapperMute} instance, as well as
     ...
     * <p>
     * To do this, tracks are constructed as follows. The {@link HlsChunkSourceMute} exposes (N+1) tracks,
     ...
     * create a third set, which is the set exposed by this {@link HlsSampleStreamWrapperMute}:
     * <ul>
    ...
    private HlsMediaChunkMute getLastMediaChunk() {
    ...
    private static boolean isMediaChunk(Chunk chunk) {
        return chunk instanceof HlsMediaChunkMute;
    }
